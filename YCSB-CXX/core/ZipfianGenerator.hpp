...
class ZipfianGenerator : public NumberGenerator {
   ...
   * @param zipfianconstant The zipfian constant to use.
   */
	ZipfianGenerator(uint64_t min, uint64_t max, double zipfianconstant)
		: ZipfianGenerator(min, max, zipfianconstant, zetastatic(max - min + 1, zipfianconstant))
	{
	}
   ...
   */
	ZipfianGenerator(uint64_t min, uint64_t max, double zipfianconstant, double zetan)
		: items(max - min + 1), base(min), zipfianconstant(zipfianconstant), zetan(zetan)
	{
